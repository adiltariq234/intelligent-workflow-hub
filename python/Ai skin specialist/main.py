...
from voice_of_the_doctor import convert_text_to_audio_doctor
...
def process_inputs(audio_filepath, image_filepath, video_filepath):
    ...
    if not image_filepath:
        raise gr.Error("Please upload a skin image before analysis. Video analysis is not supported yet.")

    try:
        patient_text = transcribe_patient_voice(audio_filepath)
    except Exception as e:
        raise gr.Error(f"Could not transcribe your voice: {e}")
    ...
    try:
        ...
        raise gr.Error(f"Could not analyze the image: {e}")

    try:
        doctor_audio = convert_text_to_audio_doctor(doctor_text)
    except Exception as e:
        raise gr.Error(f"Could not generate voice response: {e}")
